...
class JenjangWidget extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        if ($role[0] == 'MM Daerah') {
            ...
            $data[1] = Mudamudi::query()->where('daerah_id', $role[1]->id)->where('status', 'Pelajar SMA')->orWhere('daerah_id', $role[1]->id)->where('status', 'Pelajar SMK')->count();
            ...
            $data[1] = Mudamudi::query()->where('daerah_id', $role[2]->daerah_id)->where('desa_id', $role[2]->id)->where('status', 'Pelajar SMA')->orWhere('daerah_id', $role[2]->daerah_id)->where('desa_id', $role[2]->id)->where('status', 'Pelajar SMK')->count();
            ...
            $data[1] = Mudamudi::query()->where('daerah_id', $role[2]->daerah_id)->where('desa_id', $role[2]->id)->where('kelompok_id', $role[3]->id)->where('status', 'Pelajar SMA')->orWhere('daerah_id', $role[2]->daerah_id)->where('desa_id', $role[2]->id)->where('kelompok_id', $role[3]->id)->where('status', 'Pelajar SMK')->count();
